...
Our code can be used and evaluated in the same way as the base implementation. The only difference is that for the sampling we suggest to use `--classifier_scale 0.15` for unconditional models and `--classifier_scale 0.025` for conditional. The remaining part of the README comes directly from the original repo, but it is fully valid for our implementation as well.   
